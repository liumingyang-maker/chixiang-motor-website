...
await book('Phase_4_Country_Product_Demand_Ranking_v6.xlsx',[{name:'Engine Top 3',headers:['Country','Rank','Product','Total','Tier','Evidence IDs','Market confidence','Engine confidence','Compatibility confidence','Meaning'],rows:data.ranking.map(r=>[r.Country,r.Rank,r.Product,r.Total,r.Tier,r['Evidence IDs'],r['Market confidence'],r['Engine confidence'],r['Compatibility confidence'],r.Total===0?'Zero-score placeholder; no evidence-supported direction':r.Country==='Colombia'?'Relative research direction; demand not verified':'Frozen Phase 4 direction'])},{name:'Aftermarket Offer',headers:['Country','Offer','Route','Status','Ads Launch'],rows:data.countries.map(c=>[c,'Engine parts / spares pack',c==='Colombia'?'SEO / distributor development':'Aftermarket qualification','VALIDATE ECONOMICS','NOT APPROVED'])}]);
